...
import com.king.automata.fragment.FailureDialogFragment;
import com.king.automata.fragment.VictoryDialogFragment;
...
public class GameActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		level = getIntent().getIntExtra(ConfigHolder.SELECTED_LEVEL_VALUE, -1);
		realm = getIntent().getIntExtra(ConfigHolder.SELECTED_REALM_VALUE, -1);
		if(level >= 0) {
			currentLevel = ConfigHolder.getLevel(realm, level, this);
		}
    ...
    public void endGame(int stars) {
    	...
    	if(stars > 0) {
	    	VictoryDialogFragment dialogFragment = VictoryDialogFragment.newInstance(
	    			this, realm, level, gameBoard.moves, gameBoard.maxMoves, 
	    			gameBoard.getLivingCellCount(), gameBoard.getMinimalCellCount(), stars);
	    	dialogFragment.show(fm,"victory_dialog");
    	} else {
    		FailureDialogFragment dialogFragment = FailureDialogFragment.newInstance(
        			this, realm, level, gameBoard.moves, gameBoard.maxMoves, 
	    			gameBoard.getLivingCellCount(), gameBoard.getMinimalCellCount());
