//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, vJAXB 2.1.10 in JDK 6 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2013.03.28 at 05:37:38 PM EDT 
//


package com.king.automata.levelconfig;

import java.util.ArrayList;
import java.util.List;

import org.simpleframework.xml.Element;
import org.simpleframework.xml.ElementList;
import org.simpleframework.xml.Root;


/**
 * <p>Java class for LevelGroupType complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType name="LevelGroupType">
 *   &lt;complexContent>
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       &lt;sequence>
 *         &lt;element name="groupName" type="{http://www.w3.org/2001/XMLSchema}string"/>
 *         &lt;element name="color" type="{http://www.w3.org/2001/XMLSchema}int"/>
 *         &lt;element name="levels" type="{com/king/automata/LevelConfig}LevelConfigType" maxOccurs="unbounded"/>
 *       &lt;/sequence>
 *     &lt;/restriction>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * 
 * 
 */
@Root
public class LevelGroup {
	
	@ElementList
    protected List<LevelConfig> levels;
	
	@Element
	protected String victoryConditionsText;
	
	@Element 
	protected String dialogText1;
	
	@Element
	protected String dialogText2;
	
    /**
     * Gets the value of the levels property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the levels property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * <pre>
     *    getLevels().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link LevelConfigType }
     * 
     * 
     */
    public List<LevelConfig> getLevels() {
        if (levels == null) {
            levels = new ArrayList<LevelConfig>();
        }
        return this.levels;
    }

	public String getVictoryConditionsText() {
		return victoryConditionsText;
	}

	public void setVictoryConditionsText(String victoryConditionsText) {
		this.victoryConditionsText = victoryConditionsText;
	}

	public String getDialogText1() {
		return dialogText1;
	}

	public void setDialogText1(String dialogText1) {
		this.dialogText1 = dialogText1;
	}

	public String getDialogText2() {
		return dialogText2;
	}

	public void setDialogText2(String dialogText2) {
		this.dialogText2 = dialogText2;
	}

}
