...
public class GameBoard {
    ...
    public GameBoard(LevelConfig currentLevel) {
		...
		this.useCounter = currentLevel != null && currentLevel.isUseCounter();
		this.useMoves = currentLevel != null && currentLevel.getMoves() > 0;
    ...
    private void setUpCurrentLevel() {
    	...
    	if(this.useCounter) {
			this.counter = currentLevel.getCounter();
			this.maxCounter = this.counter;
		}
    	if(this.useMoves) {
    		this.moves = currentLevel.getMoves();
	...
	public boolean cellExists(Location location) {
		Location newLocation = new Location(
				Math.round(location.x), 
				Math.round(location.y), 0.0f);
		return livingCells.containsKey(newLocation);
	}
	...
	public int getEscapedCellCount() {
		int escapedCellCount = 0;
		for(Location location : livingCells.keySet()) {
			if(!boardContains(location.x, location.y)) escapedCellCount++;
		}
		return escapedCellCount;
	}
	
	public boolean boardContains(float x, float y) {
		float negativeWidth = boardWidth * -1f;
		return x < boardWidth
		&& y < boardWidth
		&& x > negativeWidth
		&& y > negativeWidth;
	...
	public int getMinimalCellCount() {
		return currentLevel.getMinimumCellCount();
	}
}
