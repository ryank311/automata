...
public class GameBoardRenderer implements GLSurfaceView.Renderer{
	...
	private void drawText(GL10 gl) {
		...
		if(width < height) {
			...
			gl.glTranslatef(5.2f, -1.60f / ratio, -6f);
		}
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		glText.load( "Roboto-Regular.ttf", 48, 2, 2 );  // Create Font (Height: 14 Pixels / X+Y Padding 2 Pixels)
	}
	...
	public void translate(float x, float y) {
		if(width < height) {
			mTranslateX += x * TOUCH_SCALE_FACTOR / width * zoom * ratio;
	        mTranslateY -= y * TOUCH_SCALE_FACTOR / height * zoom;
		}
		else {
			mTranslateX += x * TOUCH_SCALE_FACTOR / width * zoom;
	        mTranslateY -= y * TOUCH_SCALE_FACTOR / height * zoom * ratio;
		}
	}
